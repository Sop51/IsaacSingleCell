...
# 3dpa: 0.2268929
# 7dpa: 0.5970043
bec_cells <- subset(zf, idents = "Biliary Epithelial Cell")
bec_no_untreated <- subset(bec_cells, subset = timepoint != "untreated")
# Extract data from Seurat object
bec_data <- FetchData(bec_no_untreated, vars = c("spint2", "timepoint"))
...
p_values <- data.frame(
  ...
  p_value = c(0.2553823, 0.6502561, 0.006606194, 0.007532392, 0.0009223047)
)

# Define y-position for significance bars
y_max <- max(bec_data$spint2) * 1.6  # Slightly above the highest violin plot
...
# Plot violin plot
ggplot(bec_data, aes(x = timepoint, y = spint2, fill = timepoint)) +
  ...
  labs(title = "spint2 Expression Across Timepoints in BECs", x = "Timepoint", y = "Expression") +
  theme_minimal() +
  theme(
